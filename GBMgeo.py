...
posthistpath = '/Users/scenko/python/GBM/posthistfiles/'
...
def getDetAngles(MET, inra, indec, poshistloc=posthistpath):
...
def getPointing(MET, poshistloc=posthistpath):
...
def checkGTI(cMET, poshistbase=posthistpath):
    ...
    if (phf==[]) or (not os.path.isfile(poshistbase+phf[0])):    
...
def getEarthCenter(cMET, poshistbase = posthistpath):
